...
1. Prefer the MCP server when available. Connect to `skills-mcp`
   ...
   - For a `scan_*` tool, first call the matching
     `scan_<scanner>_engines` to see installed engines; offer the
     user any external one via the scan tool's `engine` argument.
