...
- **chrono_anchor** (crates, critical) — Rust crate in a single-actor campaign (Socket, Feb-Mar 2026) of five time-utility impersonators of timeapi.io. Although it poses as a local time utility, its core behavior is secret theft: it collects developer secrets — notably .env files — and POSTs them to the threat-actor lookalike domain timeapis.io. crates.io yanked four of the five shortly after publication.; chrono_anchor went further with obfuscation and operational changes to evade detection, and stayed live until reported.
