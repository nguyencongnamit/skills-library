...
- **org.fasterxml.jackson.core:jackson-databind** (maven, critical) — Typosquat of the ubiquitous Jackson JSON library (legitimate groupId com.fasterxml.jackson.core) using a namespace-prefix swap to org.fasterxml.jackson.core — mirroring a lookalike domain fasterxml.org (vs the real fasterxml.com) registered 2025-12-17. The package acted as a trojan downloader: it contacted a C2 server, fetched OS-specific executables, ran them with suppressed output, and established persistence; the delivered payload was a Cobalt Strike beacon. Removed from Maven Central within ~1.5 hours of reporting (Aikido Security, 2025-12-25).
- **faster_log** (crates, critical) — Typosquat of the popular fast_log logging crate. Published 2025-05-25 by the accounts 'rustguruman' and 'dumbnbased', it shipped working logging code as cover while a hidden routine scanned local Rust source files for Solana and Ethereum private keys and exfiltrated matches via HTTP POST to a hardcoded Cloudflare Workers C2 endpoint. About 8,424 downloads across this crate and async_println before crates.io removed them.
