...
- **spl-token-py** (pypi, critical) — Part of the JFrog-disclosed 'Solana FakeFix' campaign (2026-06-11): ~25 npm/PyPI packages impersonating Solana developer tooling (@solana/web3.js, @solana/spl-token) and luring developers with fake 'stable/fixed/patched/LTS' builds. On install/import it steals Solana keypairs (~/.config/solana/id.json), SSH keys (~/.ssh/id_rsa), AWS credentials (~/.aws/credentials), source-control tokens, and .env secrets (vars named KEY/SECRET/MNEMONIC/TOKEN).
